...
var dataAlreadyLoadedFromNetwork = false;
fetch(url)
  .then(function(res) {
    return res.json();
  })
  .then(function(data) {
    console.log('From Web data ', data.origin);
    dataAlreadyLoadedFromNetwork = true;
...
if('caches' in window) {
  caches.match(url).then(resp => {
    ...
    if(!dataAlreadyLoadedFromNetwork) {
      console.log('From Cache data ', data.origin);
      box.style.height = (data.origin.substr(0, 2) * 5) + 'px';
    }
  })
}
